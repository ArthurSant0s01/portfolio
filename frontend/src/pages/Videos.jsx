        ...
        <div className="mt-14 pb-24">
          {!hasVideos ? (
            <EmptyGallery
              ...
              from="#3B82F6"
              to="#8B5CF6"
              title="Showreel in production"
              description="This space is ready for cinematic edits, showreels and short films. New videos will appear here soon."
            />
          ) : (
      ...
      {active && (
        ...
        >
          <motion.div initial={{ scale: 0.94, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative w-full max-w-4xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setActive(null)} data-testid="video-modal-close" className="absolute -top-12 right-0 w-10 h-10 rounded-full glass-strong flex items-center justify-center hover:bg-white hover:text-black transition-colors">
              <X size={18} />
            </button>
            <div className="rounded-2xl overflow-hidden glass-strong">
              <video className="w-full aspect-video" controls autoPlay poster={active.thumbnail}>
                {mediaConfig.sampleVideo && <source src={mediaConfig.sampleVideo} type="video/mp4" />}
              </video>
              <div className="p-5">
